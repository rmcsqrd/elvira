...
void i2c_write_wrapper(uint8_t slave_address, uint8_t slave_register, uint8_t register_value){
    
    // initiate I2C bus
    int bus = i2c_open("/dev/i2c-1");  // hardcoded for I2C bus 1 because I wired it that way
    I2CDevice device;
    memset(&device, 0, sizeof(device));

    // configure bus
    device.bus = bus;
    device.addr = slave_address;
    device.iaddr_bytes = 1; // slave address is 1 byte
    device.page_bytes = 2;  // slave register address, register data
    
    unsigned char buffer[1];
    ssize_t size = sizeof(buffer);
    memset(buffer, register_value, sizeof(buffer));  // pack buffer with data

    i2c_write(&device, slave_register, buffer, size);  // write data
    i2c_close(bus);
}
